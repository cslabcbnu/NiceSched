...
/* for calculate task cpu time
 * Enhacing NiceSched method
 */
u64 task_cpu_usage(struct task_struct *p) {
    u64 ut, st, total_time, elapsed_time, cpu_usage;
    u64 system_uptime = ktime_get_coarse_boottime_ns();
    ...
    if (p->prev_cputime.utime || p->prev_cputime.stime) {
        total_time -= (p->prev_cputime.utime + p->prev_cputime.stime);
    }

    elapsed_time = system_uptime - p->prev_systime;
    printk("[%s] %d ; total_time: %llu, elapsed_time: %llu second : %d", p->comm, p->pid, total_time, elapsed_time,total_time/NSEC_PER_SEC);

    // 이전 값 갱신
    p->prev_cputime.utime = ut;
    p->prev_cputime.stime = st;
    p->prev_systime = system_uptime;

    if (elapsed_time > 0) {
        cpu_usage = (total_time * 100) / (elapsed_time);
        printk("[%s] %d ; cpu usage: %llu%%", p->comm, p->pid, cpu_usage);
        return cpu_usage;
    }
...
int calc_nice_from_lr_ratio(int lr_ratio)
{
	if (lr_ratio <=5)
		return 0;
	return 20 - (lr_ratio*3.9)
}
...
//static const int nice_table[11]={ -20, -16, -12, -8, -4, 0, 4, 8, 12, 16, 19};
//static const int nice_table[11]={ 19, 16, 12, 8, 4, 0, -4, -8, -12, -16, -20};
static const int nice_table[11]={ 0, 0, 0, 0, 0, 0, -4, -8, -12, -16, -20};
/*
...
//			return;
		...
		set_user_nice(p,calc_nice_from_lr_ratio(lr_ratio));
...
void task_numa_fault(int last_cpupid, int mem_node, int pages, int flags)
{
	...
	*/
	...
	p->numa_faults_locality[local+3] += pages;
}
